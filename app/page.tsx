...
export default function Home() {
  ...
  async function handleExportExcel() {
    ...
    try {
      ...
      // ── แจ้งผลสำเร็จด้วย toast แทน alert ────────────────────
      if (isWebView2 || dirHandle) {
        toast({
          title: "✅ บันทึกสำเร็จ",
          description: `บันทึกในโฟลเดอร์ "${folderName}" บน Desktop\n• MobileCheckUp.xlsx\n• Result.xlsx`,
          variant: "success",
        });
      } else {
        toast({
          title: "✅ ดาวน์โหลดสำเร็จ",
          description: "MobileCheckUp.xlsx และ Result.xlsx",
          variant: "success",
        });
      }
    ...
    <>
      ...
      <main className="app-shell">
        <header className="page-header">
          <div>
            <h1>ระบบลงทะเบียน{detailName ? ` · ${detailName}` : ""}</h1>
            <p>Team 1</p>
          </div>
          <div
