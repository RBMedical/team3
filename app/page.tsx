...
export default function Home() {
  ...
  async function handleExportExcel() {
    ...
    try {
      ...
      // ── ชื่อ folder จาก Sheet "Detail" cell A1 ───────────────
      const folderName = (exportRes.folderName || "MobileCheckUp").replace(/[\\/:*?"<>|]/g, "_");
      console.log("[Export] folderName from API:", exportRes.folderName);
      console.log("[Export] folderName after sanitize:", folderName);
      console.log("[Export] isWebView2:", !!(window as any).__isWebView2__);
      ...
      async function saveViaWebView2(files: { name: string; data: ArrayBuffer }[], folder: string): Promise<string> {
        return new Promise((resolve, reject) => {
          ...
          const payload = {
            ...
            files: files.map(f => ({
              name: f.name,
              data: btoa(String.fromCharCode(...new Uint8Array(f.data))),
            })),
          };
