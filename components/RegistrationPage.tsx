...
// Detail section — 2 คอลัมน์ (HN และ ชื่อ นามสกุล แสดงในการ์ดหัวด้านบนแล้ว)
const FIELDS: (keyof EmployeeRow)[] = [
  "รหัสประจำตัว","เลขบัตรประชาชน",
  "แผนก","ตำแหน่ง","ชั้นปี","สาขา","ห้อง","โปรแกรม",
  "Customer","ลำดับลงทะเบียน","วันที่ลงทะเบียน","เวลาลงทะเบียน","หมายเหตุ",
];
...
export function RegistrationPage({ onCountsUpdate, onOpenPersonal, detailName = "", staffName = "" }: Props) {
  ...
  async function handleSearch(searchTerm?: string, opts?: { suppressNotFound?: boolean }) {
    const q = (searchTerm ?? query).trim();
    if (!q) return setStatusMsg("กรุณาระบุคำค้นหา", false);
    // sync ช่องค้นหาให้ตรงกับคำที่ใช้ค้นจริง
    if (searchTerm !== undefined) setQuery(q);
    setStatusMsg("กำลังค้นหา...");
    try {
      const result = await appScriptRequest<SearchResponse>({ action: "search", q }, "กำลังค้นหา...");
  ...
  async function handleDelete() {
    ...
    try {
      const result = await appScriptRequest<{ ok: boolean; message?: string }>({ action: "delete", hn }, "กำลังลบ...");
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .hn { font-size: 2mm; font-weight: 700; color: #000; white-space: nowrap; line-height: 1; font-family: 'Sarabun', sans-serif; letter-spacing: 0.05em; }
  .name-row {
    padding: 0.2mm 0.8mm;
...
<script>
  window.onload = function() {
    document.querySelectorAll('.bc').forEach(function(el) {
      ...
      if (!val) return;
      try {
        JsBarcode(el, val, { format:'CODE39', displayValue:true, font:'Sarabun', fontSize:9, textMargin:1, textAlign:'left', margin:0, width:1.3, height:24 });
      } catch(e) {}
    });
...
<\/script>
    ...
    // ใช้ Blob URL แทน document.write เพื่อหลีกเลี่ยง cross-origin block
    const blob = new Blob([html], { type: "text/html;charset=utf-8" });
    const url  = URL.createObjectURL(blob);
    const w    = window.open(url, "_blank", "width=900,height=700");
    if (!w) { setStatusMsg("ป๊อปอัพถูกบล็อก กรุณาอนุญาตป๊อปอัพ", false); }
  ...
  return (
    <>
      <section className="register-panel">
        ...
        {/* Result list — dropdown when multiple results */}
        {showResults && (
        ...
        {/* Selected row card — HN + ชื่อ-นามสกุล */}
        <div className="selected-row-card">
          <div className="src-icon">
            <span className="src-minus">—</span>
          </div>
          <div className="src-info">
            <div className="src-name">{currentRow ? (currentRow["ชื่อ นามสกุล"] || "ค้นหาผู้รับการตรวจ") : "ค้นหาผู้รับการตรวจ"}</div>
            <div className="src-sub">{currentRow ? `HN ${currentRow.HN || "-"}` : "กรอกหรือสแกนบาร์โค้ดเพื่อดึงข้อมูล"}</div>
          </div>
          <div className="src-meta">
            <span className="src-meta-label">รหัสลงทะเบียน</span>
            <span className="src-meta-value">{currentRow?.["รหัสประจำตัว"] || "—"}</span>
          </div>
          <div className="src-meta">
            <span className="src-meta-label">วันที่ตรวจ</span>
            <span className="src-meta-value src-meta-blue">{currentRow?.["วันที่ลงทะเบียน"] || new Date().toLocaleDateString("th-TH", { day: "numeric", month: "short", year: "numeric" })}</span>
          </div>
        </div>
          ...
          <div className="count-grid">
            {[
              ...
              <div className="count-field" key={id}>
                ...
                <input readOnly value={counts[id] ?? 0} />
              </div>
            ))}
          </div>
        </div>
          ...
          <table className="queue-table">
            ...
            <tbody>
              {queueRows.map((row, i) => (
                <tr
                  key={i}
                  className={currentRow && String(currentRow.HN).trim() === String(row.hn).trim() ? "queue-row-selected" : ""}
                  onClick={() => handleSearch(row.hn, { suppressNotFound: true })}
                >
      ...
      <AddNewModal
        ...
        onSuccess={(name) => {
          ...
          loadQueue();
          // ค้นหารายการที่เพิ่งเพิ่มโดยอัตโนมัติด้วยชื่อที่กรอก
          handleSearch(name, { suppressNotFound: true });
        }}
      />
