...
const FIELDS: (keyof EmployeeRow)[] = [
  ...
  "ลำดับลงทะเบียน","วันที่ลงทะเบียน","เวลาลงทะเบียน","Customer","หมายเหตุ",
];

// ฟิลด์แคบ = span 1 คอลัมน์ (ที่เหลือ span 2) ใน grid 4 คอลัมน์
const NARROW_FIELDS = new Set<string>([
  "ชั้นปี","สาขา","โปรแกรม","ลำดับลงทะเบียน","วันที่ลงทะเบียน","เวลาลงทะเบียน",
]);
...
export function RegistrationPage({ onCountsUpdate, onOpenPersonal, detailName = "" }: Props) {
  ...
  async function handleSearch(searchTerm?: string, opts?: { suppressNotFound?: boolean }) {
    ...
    try {
      const result = await appScriptRequest<SearchResponse>({ action: "search", q });
      ...
      <section className="register-panel">
        ...
        <div className="selected-row-card">
          ...
          <div className="src-meta">
            <span className="src-meta-label">รหัสประจำตัว</span>
        ...
        <form className="data-form" onSubmit={(e) => e.preventDefault()}>
          {FIELDS.map((field) => (
            <div
              className={`field ${NARROW_FIELDS.has(field as string) ? "field--c1" : "field--c2"}`}
              key={field}
            >
              <label>{field}</label>
              <input
