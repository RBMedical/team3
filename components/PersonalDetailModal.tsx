...
export function PersonalDetailModal({ open, initialHn = "", onClose, onGoFinance }: Props) {
  ...
  async function loadTestNames() {
    try {
      const res = await appScriptRequest<{ ok: boolean; names?: string[]; items?: TestNameItem[] }>({ action: "getTestNames" });
      if (res.ok) {
        setTestNames(res.names || []);
        setTestNameItems(res.items || []);
      }
    } catch {}
  }
                        ...
                        <div key={i} style={{
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: "0.72rem", fontWeight: 600, color: "#4a2d80", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                              {ex.รายการ}
                              {ex.ราคา && (
                                <span style={{ fontWeight: 400, color: "#7c3aed", marginLeft: 5 }}>
                                  ({Number(ex.ราคา).toLocaleString("th-TH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })})
                                </span>
                              )}
                            </div>
                            {ex.วันที่ && (
                          ...
                          {addItems.map((val, idx) => (
                            <div key={idx} style={{ display: "flex", gap: 5, marginBottom: 5, alignItems: "center" }}>
                              ...
                              >
                                ...
                                {testNameItems.length > 0
                                  ? testNameItems.map((item, ni) => (
                                      <option key={ni} value={item.name}>
                                        {item.name}{item.price ? ` (${Number(item.price).toLocaleString("th-TH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })})` : ""}
                                      </option>
                                    ))
