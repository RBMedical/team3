...
export function AddNewModal({
  ...
  useEffect(() => {
    if (open) {
      setValues(
        Object.fromEntries(
          modalInputFields.map((f) => [f, f === "Customer" ? prefillCustomer : ""])
        ) as FormValues
      );
      setStatus(null);
    }
  ...
  async function handleSubmit() {
    ...
    const program = values["โปรแกรม"].trim();
    if (!program) {
      setStatus({ msg: "กรุณากรอกโปรแกรม", ok: false });
      return;
    }
          ...
          <div className="modal-grid">
            ...
            {(
              ...
              <div className="modal-field" key={field}>
                <label>
                  {field}
                  {field === "โปรแกรม" && <span className="required"> *</span>}
                </label>
                <input
                  value={values[field]}
                  onChange={(e) =>
                  ...
                  placeholder={field === "โปรแกรม" ? "โปรแกรม" : "-"}
                />
              </div>
            ))}
